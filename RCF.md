...
A validação DEVE comprovar: inventário completo e reprodutível a partir do release materializado; diferença vazia entre obrigações detectadas e registros publicados; ausência de dependências e créditos especulativos; textos, autores, versões, links, modificações e licenças exigidos; preservação dos avisos dentro dos artefatos quando aplicável; HTML válido; landmark, título, ordem de headings, contraste, teclado e nome acessível; viewports de 320, 768 e 1280 px; impressão A4; build limpo e determinístico; allowlist do site; rota local `/atribuicoes` e `/atribuicoes/`; execução do workflow; deployment Pages concluído; resposta pública HTTP 200 na URL canônica; e ausência de regressão nas páginas, módulos, release, GUI e bundle offline existentes.
