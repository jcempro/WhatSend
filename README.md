...
`${remetente}` é uma variável reservada da GUI Node. Quando presente em qualquer modelo aberto, ela exige um nome composto apenas por letras Unicode e espaços e o normaliza como `*_Nome:_*` somente na renderização do envio; uma coluna CSV de mesmo nome não a substitui. A CLI não coleta esse valor e rejeita antes do envio modelos que dependam do marcador.
...
O editor da GUI não salva HTML nem formato rico. Os comandos ficam em exatamente duas barras horizontais: documento/persistência/importação/exportação e composição/formatação/expressões/fluxo. A segunda barra apenas insere ou remove texto compatível com WhatsApp, incluindo formatação, `${}`, `${remetente}`, `ultimaconversa`, funções condicionais/lógicas/matemáticas, `if/else`, `$pause$`, `$diatarde$`, `$postagem$` e `^^^`. Linhas `^^^` viram abas visuais automaticamente; ao salvar ou enviar, as abas são recombinadas com o mesmo separador. O campo de remetente existe somente na GUI Node e aparece enquanto qualquer modelo aberto usar o marcador. A prévia lateral mostra somente a aba ativa, mantém rolagem sincronizada e prioriza valor real, resultado determinístico, exemplo fictício inequívoco — `Sicrano` para o remetente — e, por último, a notação literal; o offline não recebe o campo Node.
