...
function buildOfflineBundle(outputDir) {
  ...
  const html = renderOfflineHtml({ advisorySource, csvSource, favicon, packageSource, parityManifest, tabulatorCss, tabulatorLicense, tabulatorSource });
...
</div><div class="toolbar" aria-label="Ferramentas de edição textual"><button type="button" data-wrap="*" aria-label="Negrito"><strong>B</strong></button><button type="button" data-wrap="_" aria-label="Itálico"><em>I</em></button><button type="button" data-wrap="~" aria-label="Tachado"><s>S</s></button><button type="button" data-wrap="&#96;&#96;&#96;" aria-label="Monoespaçado">Código</button><button type="button" data-insert="$diatarde$">Saudação por horário</button><button type="button" data-insert="$postagem$">Nova postagem</button><button type="button" data-insert="&#10;&#10;^^^&#10;&#10;">Novo modelo</button><button type="button" id="insertVariable">Inserir variável</button><label for="emojiSelect" class="hint">Emoji</label><select id="emojiSelect" aria-label="Inserir emoji profissional"><option value="">Escolher</option><option>✅</option><option>⚠️</option><option>📌</option><option>📎</option><option>📅</option><option>💬</option><option>👍</option><option>🙏</option></select><label class="file-button">Incorporar anexo<input id="embedAttachment" type="file"></label></div><div id="modelTabs" class="model-tabs" aria-label="Blocos do modelo"></div><div class="layout"><div><label for="templateText"><strong>Texto do modelo</strong></label><textarea id="templateText" spellcheck="false" autocomplete="off" autocapitalize="off" placeholder="$diatarde$, \${nome}."></textarea><div id="modelStatus" class="status" role="status" aria-live="polite"></div></div><div><strong>Prévia com a linha selecionada</strong><div id="preview" class="preview"></div><strong>Verificações editoriais</strong><div id="advisories" class="advisories" aria-live="polite"></div></div></div></section>
<section class="panel" aria-labelledby="csvTitle"><h2 id="csvTitle">Dados CSV</h2><p class="hint">Obrigatórios: nome e exatamente um alias telefônico (telefone ou fone). A exportação usa UTF-8 com BOM, ponto e vírgula e aspas.</p><div class="toolbar">
<label class="file-button">Abrir .csv<input id="openCsv" type="file" accept=".csv,text/csv,text/plain"></label><button id="saveCsv" type="button">Salvar .csv</button><button id="addRow" type="button">Adicionar linha</button><button id="deleteRows" type="button">Excluir selecionadas</button><button id="addColumn" type="button">Adicionar coluna</button><button id="renameColumn" type="button">Renomear coluna</button><button id="removeColumn" type="button">Remover coluna</button><button id="clearStorage" type="button">Limpar dados locais</button>
...
<script>
(function(){"use strict";
var STORAGE_KEY="whatsend.offline-editor.v1";var NAMED_KEY="whatsend.offline-editor.v1.named.";var headers=["nome","telefone"];var preservedPackage=null;var templateText=document.getElementById("templateText");var modelStatus=document.getElementById("modelStatus");var csvStatus=document.getElementById("csvStatus");var advisories=document.getElementById("advisories");var preview=document.getElementById("preview");var modelTabs=document.getElementById("modelTabs");
...
function setStatus(element,message,type){element.textContent=message||"";element.className="status"+(type?" "+type:"");}
...
function renderPreview(){var selected=table.getSelectedData();var row=selected[0]||table.getData()[0]||{};var byName={};Object.keys(row).forEach(function(key){byName[WhatSendCsv.normalizeHeader(key)]=String(row[key]==null?"":row[key]);});var text=templateText.value.replace(/\\$\\{([A-Za-z_][\\w.-]*)\\}/gu,function(all,name){var normalized=WhatSendCsv.normalizeHeader(name);if(normalized==="telefone"||normalized==="fone")return byName.telefone||byName.fone||"";return Object.prototype.hasOwnProperty.call(byName,normalized)?byName[normalized]:all;});var hour=new Date().getHours();text=text.replace(/\\$diatarde\\$/giu,hour<12?"bom dia":hour<18?"boa tarde":"boa noite");preview.innerHTML=previewHtml(text||"A prévia aparecerá aqui. Selecione uma linha do CSV para conferir as substituições.");}
function renderModelTabs(){var source=templateText.value;var starts=[0];var pattern=/^\\s*\\^\\^\\^\\s*$/gmu;var match;while((match=pattern.exec(source)))starts.push(match.index+match[0].length);modelTabs.innerHTML="";starts.forEach(function(start,index){var button=document.createElement("button");button.type="button";button.textContent="Modelo "+(index+1);button.addEventListener("click",function(){var end=index+1<starts.length?starts[index+1]:source.length;templateText.focus();templateText.setSelectionRange(start,end);});modelTabs.append(button);});}
function renderAdvisories(){var issues=WhatSendAdvisory.analyzeTemplate(templateText.value);renderPreview();renderModelTabs();advisories.innerHTML="";templateText.classList.toggle("editor-alert",issues.length>0);if(!issues.length){advisories.textContent="Nenhuma ocorrência ativa.";}issues.forEach(function(issue){var button=document.createElement("button");button.type="button";button.className="issue "+issue.severity;button.textContent=(issue.severity==="error"?"Erro":"Aviso")+" — linha "+issue.line+", coluna "+issue.column+": "+issue.message;button.addEventListener("click",function(){templateText.focus();templateText.setSelectionRange(issue.index,issue.index+issue.length);});advisories.append(button);});saveState();}
...
document.getElementById("openMd").addEventListener("change",async function(event){var file=event.target.files[0];if(!file)return;if((templateText.value||table.getData().length)&&!confirm("Substituir o modelo atual?")){event.target.value="";return;}templateText.value=await readFile(file);preservedPackage=null;renderAdvisories();setStatus(modelStatus,"Modelo carregado: "+file.name,"ok");event.target.value="";});
...
document.getElementById("openPackage").addEventListener("change",async function(event){var file=event.target.files[0];if(!file)return;try{var parsed=await WhatSendPackage.parsePackage(await readFile(file));WhatSendCsv.parseCsv(parsed.csvContent);if((templateText.value||table.getData().length)&&!confirm("Substituir atomicamente o modelo e o CSV atuais?")){event.target.value="";return;}await applyCsv(parsed.csvContent);templateText.value=parsed.templateContent;preservedPackage=parsed.document;renderAdvisories();setStatus(modelStatus,"Pacote íntegro carregado: "+file.name,"ok");}catch(error){setStatus(modelStatus,error.message,"error");}event.target.value="";});
document.getElementById("saveMd").addEventListener("click",function(){renderAdvisories();download("modelo-whatsend.md",templateText.value,"text/markdown;charset=utf-8");});
...
document.getElementById("insertVariable").addEventListener("click",function(){var name=prompt("Nome da coluna ou variável:","nome");if(name&&/^[A-Za-z_][\\w.-]*$/u.test(name.trim()))replaceSelection("","","\${"+name.trim()+"}");else if(name)setStatus(modelStatus,"Nome de variável inválido.","error");});
document.getElementById("emojiSelect").addEventListener("change",function(event){if(event.target.value)replaceSelection("","",event.target.value);event.target.value="";});
document.getElementById("embedAttachment").addEventListener("change",function(event){var file=event.target.files[0];if(!file)return;if(file.size>10*1024*1024){setStatus(modelStatus,"Anexo excede o limite offline de 10 MiB.","error");event.target.value="";return;}var reader=new FileReader();reader.onerror=function(){setStatus(modelStatus,"Não foi possível incorporar o anexo.","error");};reader.onload=function(){var safeName=file.name.replace(/[^A-Za-z0-9._ -]/gu,"_");replaceSelection("","","!["+safeName+"]("+String(reader.result||"")+")");setStatus(modelStatus,"Anexo incorporado ao modelo.","ok");};reader.readAsDataURL(file);event.target.value="";});
document.getElementById("saveLocal").addEventListener("click",function(){var name=prompt("Nome do salvamento local:","modelo");if(!name)return;name=name.trim();if(!/^[\\p{L}\\p{N}._ -]{1,80}$/u.test(name)){setStatus(modelStatus,"Nome de salvamento inválido.","error");return;}localStorage.setItem(NAMED_KEY+name,JSON.stringify(localSnapshot()));setStatus(modelStatus,"Salvamento local atualizado: "+name+".","ok");});
document.getElementById("openLocal").addEventListener("click",function(){var names=Object.keys(localStorage).filter(function(key){return key.indexOf(NAMED_KEY)===0;}).map(function(key){return key.slice(NAMED_KEY.length);}).sort();if(!names.length){setStatus(modelStatus,"Nenhum salvamento local disponível.","error");return;}var name=prompt("Informe o nome a abrir:\\n"+names.join("\\n"),names[0]);if(!name)return;if((templateText.value||table.getData().length)&&!confirm("Substituir atomicamente a edição atual?"))return;try{var state=JSON.parse(localStorage.getItem(NAMED_KEY+name)||"null");applySnapshot(state).then(function(){setStatus(modelStatus,"Salvamento local aberto: "+name+".","ok");});}catch(error){setStatus(modelStatus,error.message,"error");}});
document.getElementById("deleteLocal").addEventListener("click",function(){var name=prompt("Nome exato do salvamento local a excluir:","");if(!name)return;if(!localStorage.getItem(NAMED_KEY+name)){setStatus(modelStatus,"Salvamento local não encontrado.","error");return;}if(confirm("Excluir o salvamento local "+name+"?")){localStorage.removeItem(NAMED_KEY+name);setStatus(modelStatus,"Salvamento local excluído.","ok");}});
document.getElementById("clearAll").addEventListener("click",function(){if(!confirm("Limpar a edição atual?"))return;templateText.value="";headers=["nome","telefone"];table.setColumns(columnsFor(headers));table.setData([]);preservedPackage=null;renderAdvisories();validateGrid();});document.getElementById("clearStorage").addEventListener("click",function(){if(confirm("Remover o estado salvo neste navegador?")){localStorage.removeItem(STORAGE_KEY);setStatus(csvStatus,"Dados locais removidos.","ok");}});
...
function validateOfflineBundle(html) {
  ...
  for (const required of [
    ...
    "connect-src 'none'",
    "Incorporar anexo",
    "Prévia com a linha selecionada",
    "Renomear coluna",
    "sourceParity",
...
module.exports = { OFFLINE_BUNDLE_NAME, buildOfflineBundle, buildParityManifest, validateOfflineBundle };
